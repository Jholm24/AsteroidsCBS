...
class Game {
    ...
    public void start(Stage window) throws Exception {
        destroyedText = new Text(10, 20, "Destroyed entities: 0");
        ...
        scene.setOnKeyPressed(event -> {
            if (event.getCode().equals(KeyCode.A)) gameData.getKeys().setKey(GameKeys.A, true);
            if (event.getCode().equals(KeyCode.D)) gameData.getKeys().setKey(GameKeys.D, true);
            if (event.getCode().equals(KeyCode.W)) gameData.getKeys().setKey(GameKeys.W, true);
            if (event.getCode().equals(KeyCode.SPACE)) gameData.getKeys().setKey(GameKeys.SPACE, true);
        });
        scene.setOnKeyReleased(event -> {
            if (event.getCode().equals(KeyCode.A)) gameData.getKeys().setKey(GameKeys.A, false);
            if (event.getCode().equals(KeyCode.D)) gameData.getKeys().setKey(GameKeys.D, false);
            if (event.getCode().equals(KeyCode.W)) gameData.getKeys().setKey(GameKeys.W, false);
        ...
        for (Entity e : world.getEntities()) {
            if (e instanceof IPlayer) { playerMaxHealth = e.getHealth(); break; }
        }
    ...
    public void render() {
        animationTimer = new AnimationTimer() {
            ...
            public void handle(long now) {
                if (gameStartNano == 0) gameStartNano = now;
                if (gameData.isGameOver()) { stop(); showGameOver(); return; }
    ...
    private void showGameOver() {
        // Save score to microservice
        scoringService.addScore(gameData.getDestroyedAsteroids(), elapsedSeconds);
        ...
        // Fetch top scores from microservice
        List<ScoreEntry> entries = scoringService.getTopScores();
        for (int i = 0; i < entries.size(); i++) {
            ...
            rank.setFill(rowColor); asteroids.setFill(rowColor); time.setFill(rowColor);
            ...
            grid.add(rank, 0, i); grid.add(asteroids, 1, i); grid.add(time, 2, i);
        }
    ...
    private void draw() {
        ...
        for (Entity e : world.getEntities()) {
            if (e instanceof IPlayer && playerMaxHealth > 0) {
                healthBar.setWidth(Math.max(0, e.getHealth() / playerMaxHealth) * HEALTH_BAR_WIDTH);
                break;
            }
        }
        for (Entity polygonEntity : polygons.keySet()) {
            if (!world.getEntities().contains(polygonEntity)) {
                gameWindow.getChildren().remove(polygons.get(polygonEntity));
        ...
        for (Entity entity : world.getEntities()) {
            ...
            if (entity instanceof IPlayer) {
                polygon.setFill(entity.isImmune() && System.currentTimeMillis() % 200 < 100 ? Color.WHITE : entity.getColor());
            }
        }
    }
